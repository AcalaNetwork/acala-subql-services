...
type RedeemedByUnbondProps = Omit<
  RedeemedByUnbond,
  NonNullable<FunctionPropertyNames<RedeemedByUnbond>>
>;
...
export class RedeemedByUnbond implements Entity {
  ...
  static async get(id: string): Promise<RedeemedByUnbond | undefined> {
    assert(
      ...
      "Cannot get RedeemedByUnbond entity without an ID"
    );
  ...
  static async getByAddressId(
    addressId: string
  ): Promise<RedeemedByUnbond[] | undefined> {
    ...
    return records.map((record) =>
      RedeemedByUnbond.create(record as RedeemedByUnbondProps)
    );
  }
  ...
  static create(record: RedeemedByUnbondProps): RedeemedByUnbond {
    ...
    const entity = new RedeemedByUnbond(record.id);
