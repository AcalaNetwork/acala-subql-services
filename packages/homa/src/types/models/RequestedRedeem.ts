...
type RequestedRedeemProps = Omit<
  RequestedRedeem,
  NonNullable<FunctionPropertyNames<RequestedRedeem>>
>;
...
export class RequestedRedeem implements Entity {
  ...
  static async get(id: string): Promise<RequestedRedeem | undefined> {
    assert(
      ...
      "Cannot get RequestedRedeem entity without an ID"
    );
  ...
  static async getByAddressId(
    addressId: string
  ): Promise<RequestedRedeem[] | undefined> {
    ...
    return records.map((record) =>
      RequestedRedeem.create(record as RequestedRedeemProps)
    );
  }
  ...
  static create(record: RequestedRedeemProps): RequestedRedeem {
    ...
    const entity = new RequestedRedeem(record.id);
