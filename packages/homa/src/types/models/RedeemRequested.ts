...
type RedeemRequestedProps = Omit<
  RedeemRequested,
  NonNullable<FunctionPropertyNames<RedeemRequested>>
>;
...
export class RedeemRequested implements Entity {
  ...
  static async get(id: string): Promise<RedeemRequested | undefined> {
    assert(
      ...
      "Cannot get RedeemRequested entity without an ID"
    );
  ...
  static async getByAddressId(
    addressId: string
  ): Promise<RedeemRequested[] | undefined> {
    ...
    return records.map((record) =>
      RedeemRequested.create(record as RedeemRequestedProps)
    );
  }
  ...
  static create(record: RedeemRequestedProps): RedeemRequested {
    ...
    const entity = new RedeemRequested(record.id);
