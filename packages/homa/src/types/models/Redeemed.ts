...
type RedeemedProps = Omit<
  Redeemed,
  NonNullable<FunctionPropertyNames<Redeemed>>
>;
...
export class Redeemed implements Entity {
  ...
  static async get(id: string): Promise<Redeemed | undefined> {
    assert(
      ...
      "Cannot get Redeemed entity without an ID"
    );
  ...
  static create(record: RedeemedProps): Redeemed {
    ...
    const entity = new Redeemed(record.id);
