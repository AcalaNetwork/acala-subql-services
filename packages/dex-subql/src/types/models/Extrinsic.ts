...
export class Extrinsic implements CompatEntity {
    ...
    static async getByAddressId(addressId: string, options: GetOptions<CompatExtrinsicProps>): Promise<Extrinsic[]> {
        // Inputs must be cast as the store interface has not been updated to support alternative ID types
        const records = await store.getByField<CompatExtrinsicProps>('Extrinsic', 'addressId', addressId, options);
        return records.map(record => this.create(record as unknown as ExtrinsicProps));
    }
