...
export class PriceBundle implements CompatEntity {
    ...
    static async getByTokenId(TokenId: string, options: GetOptions<CompatPriceBundleProps>): Promise<PriceBundle[]> {
        // Inputs must be cast as the store interface has not been updated to support alternative ID types
        const records = await store.getByField<CompatPriceBundleProps>('PriceBundle', 'TokenId', TokenId, options);
        return records.map(record => this.create(record as unknown as PriceBundleProps));
    }
