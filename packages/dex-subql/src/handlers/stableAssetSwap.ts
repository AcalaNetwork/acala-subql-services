...
export const stableAssetSwap = async (event: SubstrateEvent) => {
    ...
    const [swapper, poolId, a, inputAsset, outputAsset, inputAmount, minOutputAmount, balances, totalSupply, outputAmount] = event.event.data as unknown as [AccountId, number, number, string, string, Balance, Balance, Balance[], Balance, Balance];
