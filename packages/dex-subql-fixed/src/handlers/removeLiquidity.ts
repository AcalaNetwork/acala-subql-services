...
export const removeLiquidity = async (event: SubstrateEvent) => {
	...
	const [_, currency0, pool0Decrement, currency1, pool1Decrement] = event.event.data as unknown as [any, any, any, any, any];
...
export const createRemoveLiquidyHistory = async (event: SubstrateEvent, price0: FN, price1: FN) => {
	...
	const [owner, currency0, pool0Decrement, currency1, pool1Decrement, shareAmount] = event.event.data as unknown as [any, any, any, any, any, any];
	const blockData = await ensureBlock(event);
