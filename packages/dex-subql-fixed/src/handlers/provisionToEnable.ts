...
export const provisionToEnable = async (event: SubstrateEvent) => {
	...
	const [tradingPair, _token0Amount, _token1Amount, totalShareAmount] = event.event.data as unknown as [[any, any], any, any, any];
...
export const createPool = async (event: SubstrateEvent) => {
	// [trading_pair, pool_0_amount, pool_1_amount, total_share_amount\]
	const [tradingPair, _token0Amount, _token1Amount, _] = event.event.data as unknown as [[any, any], any, any, any];
...
export const createProvisionToEnableHistory = async (event: SubstrateEvent) => {
	// [trading_pair, pool_0_amount, pool_1_amount, total_share_amount\]
	const [tradingPair, token0Amount, token1Amount] = event.event.data as unknown as [[any, any], any, any];
	const blockData = await ensureBlock(event);
