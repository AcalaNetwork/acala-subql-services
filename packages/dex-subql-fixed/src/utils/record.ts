...
export const getPool = async (token0: string, token1: string, poolId?: string) => {
	...
	if (!record) {
		...
		const feeData = api.consts.dex.getExchangeFee as unknown as [any, any];
...
export const getTokenDailyData = async (id: string) => {
	...
	if (!record) {
		const newRecord = TokenDailyData.create({
			id: id,
			tokenId: "",
			amount: BigInt(0),
			tvl: BigInt(0),
			dailyTradeVolume: BigInt(0),
			dailyTradeVolumeUSD: BigInt(0),
			dailyTxCount: BigInt(0),
			timestamp: new Date(),
			updateAtBlockId: "",
			price: BigInt(0)
		});
...
export const getStableAssetSwap = async (id: string) => {
	...
	if (!record) {
		const newRecord = StableAssetSwap.create({
			id: id,
			poolId: -1,
			feeAmount: BigInt(0),
			yieldAmount: BigInt(0),
			exchangeRate: 0,
			totalStaking: BigInt(0),
