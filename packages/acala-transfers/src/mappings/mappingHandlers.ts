...
export async function handleNonNativeTokenTransfer(event: SubstrateEvent) {
    ...
    await createTransfer(from.toString(), to.toString(), getTokenName(token), BigInt((amount as Balance).toString()), event)
}
...
export async function handleNativeTokenTransfer(event: SubstrateEvent) {
    ...
    // save transfer record
    await createTransfer(from.toString(), to.toString(), getTokenName(nativeToken), BigInt((amount as Balance).toString()), event)
}
