...
The simplest way to run your project is by running `bun run dev` or `npm run-script dev`. This does all of the following:

1.  `bun run codegen` - Generates types from the GraphQL schema definition and contract ABIs and saves them in the `/src/types` directory. This must be done after each change to the `schema.graphql` file or the contract ABIs
